defmodule WmhubWeb.ProjectLive.PaymentPointersComponent do
    ...
    def pointer_partial(:initial, %ProjectsPointers{id: id, payment_pointer: pointer}, assigns) do
        ~L"""
        <li phx-click="start-edit-pointer" phx-value-project-pointer-id="<%= id %>" phx-target="<%= @myself %>"><%= pointer %></li>
        """
    end

    def pointer_partial(:editing, %ProjectsPointers{id: id}, %{edited_pointer: %{id: id, pointer_value: pointer}} = assigns) do
        ~L"""
        <%= f = form_for :edit_pointer_form, "#", [phx_submit: "save-edit-pointer", phx_change: "change-edit-pointer", phx_target: @myself] %>
            ...
            <%= submit "Save" %>
            <button type="button" phx-click="cancel-edit-pointer" class="button button-outline" phx-target="<%= @myself %>">Discard</button>
        </form>
        """
    end
    ...
    def add_new_pointer_partial(_state, _assigns), do: ""
    ...
    def handle_event("start-edit-pointer", %{"project-pointer-id" => project_pointer_id}, socket) do
        %ProjectsPointers{id: id, payment_pointer: pointer_value} = Enum.find(socket.assigns.project.payment_pointers, & &1.id == project_pointer_id)
        {:noreply, editing_pointer_assigns(socket, %{id: id, pointer_value: pointer_value})}
    end

    def handle_event("change-edit-pointer", %{"edit_pointer_form" => %{"pointer" => new_pointer_value}}, socket) do
        edited_pointer = socket.assigns.edited_pointer
        {:noreply, assign(socket, :edited_pointer, %{edited_pointer | pointer_value: new_pointer_value})}
    end
